...
- LIMS 与 MES 厂区的关系：非独立 MES 厂区（三厂小簧/一厂大簧/二厂大簧等）共用 `sMES_Home_Prod`（192.168.200.18）一个库，**重庆、无锡泽根的 sMES 是独立服务器**；sMES 通用资产（smes-621）一份共享。**LIMS 独立服务器/独立库，不在此列**，它服务实验室，是独立子域（2026-08-14 服务器拓扑确认）
