 ...
 *
 * ⚠️ 排障主因（2026-08-21 对照 XH02 定版）：H5 端按钮看不到，**先查缺菜单权限编号（PRIVTYPE=9）**
 *    （A07 派工作业/A01 派工设定等），只配平台权限编号（PRIVTYPE=0）H5 端不显示；
 *    对照组法：找能正常用该功能的群组/用户（如 HW0396→XH02）EXCEPT 对比权限差异，最快定位。
 *    菜单权限编号齐全仍无效 → 才是前端刷新：重登/清缓存/找 DS 确认"发布生效"。
