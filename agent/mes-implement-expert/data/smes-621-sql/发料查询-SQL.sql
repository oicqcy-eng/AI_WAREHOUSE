/*发料/扫码上料/进站核对 —— 工单材料领料排查三视角（通用模板，2026-08-16 v2 扩展）
 ...
 * 来源: 华纬用户提供（8-15 发料查询SQL-28.sql + 8-16 投喂 4 张截图）；表结构/口径 8-16 连库实测
 * 用途: 首站扫码上料报错排查三视角（--show 1/2/3 切换）——
 *   ① 发料核对（Tbloemomateriallist）      → U9 发料侧需求/已领/缺料，报「条码不存在MES」主查
 *   ② 扫码上料状态（TBLWIPEQPMATERIALSTATE）→ 上料动作/批次/数量对不上时查（已上料 vs 需求）
 *   ③ 报工进站核对（TBLWIPCONT_MATERIAL）  → 实际已进站消耗 vs 理论需求/已领，闭环判断
 *
 * ── 口径说明（务必先读）────────────────────────────
