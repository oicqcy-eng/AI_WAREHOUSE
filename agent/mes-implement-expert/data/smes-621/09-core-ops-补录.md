...
> **定位**：2026-08-21 连库实测发现 —— 192 表主字典（源自设计文档 20250313）**未覆盖**本目录 **编制时点（08-21）的 22 个查询模板**实际使用的 **68 张核心业务表**（报工/权限/生产批/点检/发料/工单/设备/工序等；查询清单现已扩至 25 个，新增模板用表已在 68 覆盖内）。本文件为**连库实测补录**（`INFORMATION_SCHEMA.COLUMNS` 导出，1727 列），字段说明以语义备注为主，无设计文档中文注释。
> ⚠️ **2026-08-23 甄审修正**：主字典表数原记 189，因 01 中 `tblWIPCONT_PCSNoChangeLog` 拼接节拆分为 4 表后为 **192**；本文件 68 张中 `TBLWIPCONT_RESOURCE` 实为主字典拼接错位误判（[01-wip-workorder.md](01-wip-workorder.md) 已有设计文档版），本处为其连库实测版，两处并存互补。
> ⚠️ **2026-08-23 甄审复核（覆盖分层）**：本文件 68 张中 **66 张设计文档 `SMES_621数据库设计文档20250313.html` 本就有结构**（当时补录未先查 `raw/` 原件、直接连库实测——结构无误但路径可选更优）；**真·设计文档也无仅 2 张**：`TBLUSRGROUPPRIVCONTROL`（群组控件禁用）、`TBLWIPERFBASIS`。叠加 `TBLWIPCONT_RESOURCE` 误判（01 亦有），**主字典+设计文档均无的实为 2 张**，其余 66 张断连可查 `raw/` 原件。
> **衔接**：与 [sMES核心链路-表结构与排障.md](../smes-621-sql/sMES核心链路-表结构与排障.md) 五链路配套；全量 1472 表清单与差异分析见 [sMES全量表清单.md](../smes-621-sql/sMES全量表清单.md)（580 设计文档可查/701 真·生产独有分层）。
