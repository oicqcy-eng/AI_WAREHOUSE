...
> **用途**：U9C→MES 的 IQC 来料检待检数据**出站队列**（ESB 中台 outbox 模式，与发料队列 `ESB_IssueWoItem_Queue` 同为 ESB 出站队列，但**两表结构不同**：本表 29 列、发料队列 40 列，列名无对应，勿当同构）。MES 从本表拉取待检到货单，用户只执行检验判定、不人工触发取数。
...
- 其余 **7546** 张标准用友产品表（7554 − 2.1~2.4 已实测 8 张）：**不全量沉淀**，查 [U9C全量表清单.md](U9C全量表清单.md)（7554 表地图）→ 按需 `INFORMATION_SCHEMA` 验证
