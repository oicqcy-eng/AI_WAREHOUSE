/* ============================================================
 * 一厂大簧 sMES 点检记录 —— SSMS 直接运行版（2026-08-15 导出）
 * ------------------------------------------------------------
 * 【连接信息】
 ...
 *   SQL Server 身份验证
 *
 * 【用途】一厂大簧设备点检执行情况：点检概览/按设备/明细/项目级 NG 定位，
 *         支撑设备点检执行率与点检 NG 跟进。
 *
 * 【用法】
 *   1. 顶部 @date 变量改日期，F5 一次跑全部 6 个视角
 *   2. 视角②③④⑤⑥设 @date='' 可跑全量累计
 *   3. 视角④（项目明细）最细粒度，可定位具体 NG 检查项目
...
WHERE A.EQUIPMENTNO LIKE '101-01-DH%'
...
LEFT JOIN TBLEQPEQUIPMENTBASIS EQP ON EQP.EQUIPMENTNO=A.EQUIPMENTNO
WHERE A.QCRESULT=1
  AND A.EQUIPMENTNO LIKE '101-01-DH%'
  AND (@date='' OR CONVERT(CHAR(10),A.CREATEDATE,120)=@date)
ORDER BY A.CREATEDATE;
 ...
 *    一厂实测(2026-08-15)：1005 条/60 设备/9 点检单/NG 49/NG率 4.9%。
 * 3. 【项目类型】明细 QCTYPE：0=标准值 / 1=范围值 / 2=显示信息 / 3=输入数据。
 * 4. 【一厂判定口径】设备前缀 101-01-DH（229 台）。
 *    共库多厂区并存，本文件全部视角已内建 '101-01-DH%' 前缀限定，勿改。
 * 5. 模板变更后须同步本文件：agent/mes-implement-expert/data/smes-621-sql/
