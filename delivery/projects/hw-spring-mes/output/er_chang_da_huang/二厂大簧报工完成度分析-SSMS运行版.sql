/* ============================================================
 * 二厂大簧 sMES 报工完成度与漏报分析 —— SSMS 直接运行版（2026-08-15 导出）
 * ------------------------------------------------------------
 * 【连接信息】
 *   服务器: 192.168.200.18   （总部 sMES 共库，一厂/二厂/金晟共用，非重庆独立库）
 *   数据库: sMES_Home_Prod
 *   账号:   只读账号（切勿用 sa / 写账号；注意只跑 SELECT）
 *   SQL Server 身份验证
 *
 * 【用途】定位二厂大簧"该报工没走完/漏出站"。二厂 8-14 实测 122 条/16 设备/63 批，
 *         本查询提供报工完成度 + 未完结清单，供周会定位卡点（工序/设备/批次）。
 *
 * 【用法】
 ...
 *             本版硬编码二厂前缀 F% + @date 变量，口径一致）。模板口径变更后须同步本文件。
 * ============================================================ */

DECLARE @date VARCHAR(10) = '2026-08-14';   -- 改成你要查的日期 YYYY-MM-DD（视角①②④用；③滞留是当下状态与日期无关）


/* —— ① 报工完成度总览（二厂限定；@date 指定则只统计该日；@date='' 全量） ——
 ...
 * 4. 【二厂判定口径（2026-08-15 实测确认）】二厂报工设备 = F 系前缀（FB/FS/FF/FY/FP/FJ/FH）。
 *    ⚠️ 二厂台账编号体系多样（BM/CP/CQ/DR/AH/ER/EP/EH 等，见「设备编号前缀-厂区映射.md」），
 *    但实测 8-14 共库 F% 前缀报工设备全部属于「设备类别=华纬二厂（大簧线）」，
 *    且二厂报工活跃设备全为 F 系 → 报工查询用 'F%' 限定可靠。
 *    ⚠️ 若日后二厂启用非 F 前缀设备报工，需按「设备类别=华纬二厂（大簧线）」JOIN 台账补充，勿盲目扩前缀。
 * 5. 【对账（2026-08-15 实测）】二厂 8-14：122 条/未完结 4/完成率 96.7%/
 *    16 设备/63 批/投入 335,965。共库多厂区并存，本文件全部视角已内建 'F%' 前缀限定，勿改。
